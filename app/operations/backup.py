...
# Windows-only ctypes members are intentionally absent from the non-Windows
# typeshed surface. Keep the platform boundary explicit without weakening the
# runtime branch or suppressing type checking for the rest of this module.
_platform_ctypes: Any = ctypes
...
def _acquire_windows_descriptor_lock(descriptor: int, *, exclusive: bool) -> None:
    ...
    _platform_ctypes.set_last_error(0)
    if lock_file(handle, flags, 0, 1, 0, ctypes.byref(overlapped)):
        return
    error_code = int(_platform_ctypes.get_last_error())
    error = OSError(error_code, str(_platform_ctypes.FormatError(error_code)))
...
def _release_windows_descriptor_lock(descriptor: int) -> None:
    ...
    if unlock_file(handle, 0, 1, 0, ctypes.byref(overlapped)):
        return
    error_code = int(_platform_ctypes.get_last_error())
    raise OSError(error_code, str(_platform_ctypes.FormatError(error_code)))
...
def _windows_lock_api(descriptor: int) -> tuple[Any, wintypes.HANDLE]:
    ...
    return _platform_ctypes.WinDLL(
...
def _archive_member_mode(source_file: _SourceFile) -> int:
    ...
    if os.name == "nt" and source_file.archive_path in {
        _CANONICAL_TOKEN_PATH,
        _CANONICAL_TOKEN_V2_KEYRING_PATH,
    }:
        # Windows' stat mode exposes only its read-only attribute, not the DACL,
        # and reports writable secrets as 0666. Preserve the backup contract's
        # least-privilege Unix mode for restoration onto POSIX hosts.
        return 0o600
    return mode
