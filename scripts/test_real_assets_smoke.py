...
This script verifies that ``NanoLoopApiClient`` can consume the **real** backend
(NanoLoop-Agent running on Linux / WSL / remote) without any mock or static data.
...
-----
    ...
    # Live mode — backend must be running
...
# ---------------------------------------------------------------------------
# Real asset IDs (provided by 郭境濠 — model / job / run / image)
# ---------------------------------------------------------------------------
MODEL_ID = "unet-small-balanced-v1"
JOB_ID = "job_b8320c18c76a4eb6bae6efa2b5e56105"
RUN_ID = "run_5d34cbac744d4b35b8983ec55ca32488"
IMAGE_ID = "img_06da324e847d419c97ccda9fcb83479c"
...
SAMPLE_QUERY_RESPONSE_WITH_CITATIONS: dict[str, Any] = {
    ...
    "tool_calls": [
        {
            "tool_name": "get_metric",
            "arguments": {"scope_id": RUN_ID, "metric": "mean_equivalent_diameter_nm"},
            "outcome": "success",
            "source_run_ids": [RUN_ID],
        },
    ],
...
def offline_verify_job_detail() -> bool:
    ...
    try:
        ...
        if isinstance(images, list):
            found = any(
                isinstance(img, dict) and img.get("image_id") == IMAGE_ID
                for img in images
            )
            if found:
                _pass(f"目标 image_id={IMAGE_ID} 存在")
            else:
                _fail(f"目标 image_id={IMAGE_ID} 未找到")

        if isinstance(runs, list):
            found = any(
                isinstance(r, dict) and r.get("run_id") == RUN_ID
                for r in runs
            )
            if found:
                _pass(f"目标 run_id={RUN_ID} 存在")
            else:
                _fail(f"目标 run_id={RUN_ID} 未找到")
...
def offline_verify_run_detail() -> bool:
    ...
    try:
        ...
        if isinstance(summary, dict):
            ...
            _pass(f"平均等效直径 mean_equivalent_diameter_nm={summary.get('mean_equivalent_diameter_nm')}")
...
def offline_verify_query_with_citations() -> bool:
    """Offline Step 3a: Parse sample UnifiedQueryResponse with citations — verify page=null handling."""
    ...
    try:
        ...
        for i, citation in enumerate(citations):
            ...
            if page is None:
                _pass(f"  citation[{i}] doc={doc_id} chunk={chunk_id} page=None → 无页码/全文引用 — 无异常")
            else:
...
def offline_verify_query_insufficient() -> bool:
    """Offline Step 3b: Parse sample UnifiedQueryResponse with INSUFFICIENT_EVIDENCE + empty citations."""
    ...
    try:
        ...
        if not citations and outcome == "INSUFFICIENT_EVIDENCE":
            _pass("outcome_code=INSUFFICIENT_EVIDENCE 且 citations 为空 → 输出'证据不足' — 逻辑正确")
        else:
            _fail(f"预期 INSUFFICIENT_EVIDENCE + 空 citations, 实际: outcome={outcome}, citations={len(citations)}")
            return False
...
def run_offline() -> int:
    ...
    print(f"  使用硬编码 JSON 样例验证前端解析逻辑 (无 HTTP 请求)")

    results: list[tuple[str, bool]] = []
    results.append(("解析任务详情样例", offline_verify_job_detail()))
    results.append(("解析运行结果样例", offline_verify_run_detail()))
    results.append(("解析查询样例 (含 citations, page=null)", offline_verify_query_with_citations()))
...
def verify_health(client: NanoLoopApiClient) -> bool:
    ...
    try:
        ...
        if isinstance(components, dict):
            comp_summary = ", ".join(
                f"{name}={str(v.get('status', '?')) if isinstance(v, dict) else v}"
                for name, v in components.items()
            )
        else:
            comp_summary = str(components)
...
def verify_get_job(client: NanoLoopApiClient) -> bool:
    ...
    try:
        ...
        if isinstance(images, list):
            ...
            if found_image:
                ...
                _warn(f"目标 image_id={IMAGE_ID} 未在 images 列表中找到（可能已被清理）")
        ...
        if isinstance(runs, list):
            ...
            if found_run:
                ...
                _warn(f"目标 run_id={RUN_ID} 未在 runs 列表中找到（可能已被清理）")
...
def verify_get_run(client: NanoLoopApiClient) -> bool:
...
def verify_query_analysis(client: NanoLoopApiClient) -> bool:
    ...
    try:
        ...
        if isinstance(citations, list):
            ...
            for i, citation in enumerate(citations):
                ...
                if page is None:
                    _pass(f"  citation[{i}] doc={doc_id} chunk={chunk_id} page=None (全文引用) — 无异常")
                else:
                    _pass(f"  citation[{i}] doc={doc_id} chunk={chunk_id} page={page}")
        ...
        if outcome_code == "INSUFFICIENT_EVIDENCE":
            if not citations:
                _warn("outcome_code=INSUFFICIENT_EVIDENCE 且 citations 为空 — 前端应显示'未返回材料知识引用'")
...
def run_live(base_url: str, api_key: str) -> int:
    ...
    results.append(("获取任务详情", verify_get_job(client)))
    results.append(("获取运行结果", verify_get_run(client)))
    results.append(("混合查询", verify_query_analysis(client)))
    client.close()
...
def main() -> int:
    ...
    if args.live:
        return run_live(args.base_url, args.api_key)
    else:
        return run_offline()
