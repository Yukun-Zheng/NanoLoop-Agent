...
from frontend.api_client import ApiClientError, NanoLoopApiClient, UploadPart
from frontend.components import _error_guidance
from frontend.state import health_rollup
from frontend.model_catalog import model_availability, model_is_runnable
...
def verify_models(client: NanoLoopApiClient) -> list:
    ...
    try:
        ...
        all_unavailable = all(
            str(m.get("status", "")).casefold() == "unavailable" for m in models
        )
        _record(
            "Models",
    ...
    try:
        ...
        def _make_png() -> bytes:
            ...
            for chunk_type in chunks:
                ...
                png += struct.pack(">I", len(chunk_data)) + chunk_name + chunk_data + struct.pack(">I", crc)
            return png
        ...
        _record("Upload", "upload succeeds", False, f"ApiClientError: {exc.code} (HTTP {exc.status_code}): {exc.message}")
        return None
    except Exception as exc:
...
def verify_run_submission(client: NanoLoopApiClient, job_id: str) -> None:
    ...
    try:
        ...
        _record(
            ...
            bool(exc.request_id) and exc.request_id.startswith("stub_") or exc.request_id.startswith("web_"),
        ...
        if guidance:
            _record(
                ...
                "模型" in guidance and ("不可用" in guidance or "加载" in guidance or "权重" in guidance),
                f"guidance={guidance!r}",
            )
        ...
        _record("Run", "create_runs raises ApiClientError", False, f"unexpected: {type(exc).__name__}: {exc}")
        traceback.print_exc()
...
def verify_rag_query(client: NanoLoopApiClient, job_id: str) -> None:
    ...
    try:
        ...
        _record("Query", "query raises ApiClientError", False, f"unexpected: {type(exc).__name__}: {exc}")
        traceback.print_exc()
...
def verify_insufficient_evidence(client: NanoLoopApiClient, job_id: str) -> None:
    ...
    try:
        ...
        _record("Query", "insufficient evidence as success", False, f"got error: {exc.code} (HTTP {exc.status_code}): {exc.message}")
    except Exception as exc:
        _record("Query", "insufficient evidence as success", False, f"unexpected: {type(exc).__name__}: {exc}")
        traceback.print_exc()
...
def verify_knowledge_reindex(client: NanoLoopApiClient) -> None:
    ...
    try:
        ...
        _record("Knowledge", "reindex raises ApiClientError", False, f"unexpected: {type(exc).__name__}: {exc}")
        traceback.print_exc()
...
def verify_export(client: NanoLoopApiClient, job_id: str) -> None:
    ...
    try:
        ...
        _record("Export", "export raises ApiClientError", False, f"unexpected: {type(exc).__name__}: {exc}")
        traceback.print_exc()
...
def verify_401(base_url: str) -> None:
    ...
    try:
        ...
        _record("Auth401", "health raises ApiClientError", False, f"unexpected: {type(exc).__name__}: {exc}")
        traceback.print_exc()
...
def verify_transport_error() -> None:
    ...
    try:
        ...
        _record("Transport", "health raises ApiClientError", False, f"unexpected: {type(exc).__name__}: {exc}")
        traceback.print_exc()
...
def main() -> int:
    ...
    with client:
        health_data = verify_health(client)
    ...
    if args.test_401:
        ...
        _skip("Auth401", "401 test", "not requested (use --test-401, requires stub --auth-mode key)")
