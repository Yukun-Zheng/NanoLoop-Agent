...
def create_app(*, auth_mode: str = "disabled", api_key: str = "test-secret-001") -> FastAPI:
    ...
    async def recommend_models(request: Request):
        ...
        return _success({"candidates": [], "requires_user_confirmation": True}, request)
            ...
            images.append(
                {
                    ...
                    "experiment_conditions": metadata.get("images", [{}])[0].get(
    ...
    async def get_boxes(job_id: str, image_id: str, request: Request):
        return _success({"image_id": image_id, "revision": 0, "boxes": []}, request)
    ...
    async def replace_boxes(job_id: str, image_id: str, request: Request):
        ...
        return _success({"image_id": image_id, "revision": 1, "boxes": boxes}, request)
    ...
    async def query_analysis(job_id: str, request: Request):
        ...
        # Dedicated payload used to verify the UI renderer.
    ...
    async def list_knowledge_documents(request: Request):
        return _success(
            {"documents": [], "pagination": {"limit": 50, "offset": 0, "total": 0}}, request
        )
...
def main() -> None:
    ...
    print(
        "[stub] Create run:   POST "
        f"http://127.0.0.1:{args.port}/api/v1/analyses/{{job_id}}/runs "
        "-> 503 MODEL_NOT_READY"
    )
    print(
        "[stub] Query:        POST "
        f"http://127.0.0.1:{args.port}/api/v1/analyses/{{job_id}}/query "
        "-> 503 RAG_INDEX_NOT_READY"
    )
