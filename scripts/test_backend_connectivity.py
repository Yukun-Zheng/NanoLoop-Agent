...
Checks:
  ...
  3. GET /analyses/{job}  -> job detail retrieval (real job_id)
...
from frontend.api_client import ApiClientError, NanoLoopApiClient

# --- Configuration ---------------------------------------------------------

DEFAULT_BASE_URL = "http://127.0.0.1:8000"
# Real job_id provided by backend team (郭境濠).
REAL_JOB_ID = "job_b8320c18c76a4eb6bae6efa2b5e56105"
...
def _make_client() -> NanoLoopApiClient:
    """Build a NanoLoopApiClient from environment configuration."""
    base_url = _env("NANOLOOP_API_BASE_URL", DEFAULT_BASE_URL) or DEFAULT_BASE_URL
...
def check_get_analysis(client: NanoLoopApiClient, job_id: str) -> bool:
    ...
    _print_kv("request_id", result.request_id)
    _print_kv("api_status", result.status)
    _print_kv("job_id", data.get("job_id", "?"))
    _print_kv("status", data.get("status", "?"))
    _print_kv("material", data.get("material", "?"))
...
def main() -> int:
    ...
    results = [
        check_health(client),
        check_list_models(client),
        check_get_analysis(client, REAL_JOB_ID),
    ]
