...
- 原 PR #3 的 citation 修复已经存在于 `yukun`；本包不重复修改 provider 或旧回归测试。
...
对 outcome、相关资产、禁返资产、未知 citation 和“证据不足必须零引用”执行自动判定。输出始终把
人工 citation/page/事实复核标为 `pending`，不会自动写 `passed=true`。

重启 API 后不得重新摄取或重建 embedding；复用首次 ingest 结果中的映射：
...
这些证据全部完成并由黄睿健独立复核前，`real_acceptance_completed` 必须保持 `false`。
