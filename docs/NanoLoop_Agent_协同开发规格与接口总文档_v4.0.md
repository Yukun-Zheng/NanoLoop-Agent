...
| 文档版本 | `v4.0` |
| 基线分支 | `yukun` |
| 基线提交 | `16456a30d63e42eda0e1d4b09ac0e7c223b3fd82` |
| 基线日期 | `2026-07-22` |
...
6. 完成后提交功能分支和 PR，由郑煜坤统一集成；开发者不得自行合并 `yukun` 或 `main`。
...
当前代码已具备 FastAPI、SQLite/Alembic、文件存储、任务编排、三类模型 Adapter、分析后处理、形貌统计、质量门控、RAG/Agent 框架、六页 Streamlit、Docker、备份恢复和 CI。当前提交的 GitHub Actions 已在 Python 3.11 与 3.12 各通过 1098 项测试，并完成 Ruff、严格 Mypy、OpenAPI、迁移、API/前端镜像、非 root 只读运行和备份恢复验证。
...
所有功能分支从最新全绿 `yukun` 创建，PR 也以 `yukun` 为 base；只有郑煜坤在阶段发布时把选定批次合入 `main`。
...
```text
feat/a-real-unet-large-v1
feat/b-scientific-tolerance-v1
feat/d-rag-real-assets-v1
feat/e-real-demo-workflow-v1
test/f-nondegraded-e2e-v1
...
```text
你正在开发 Yukun-Zheng/NanoLoop-Agent。基线为 origin/yukun@16456a3。
...
3. 权重、原图、GT、索引、秘密不进入公开 Git；公开 registry 不伪装 ready。
...
失败案例、风险、建议 commit message 和 PR 摘要。不要自行 commit、push 或合并。
```
...
本批目标是完成一条真实固定路径：上传→ROI→确认 ready 模型→运行→进度→质量/结果→
复核→导出→数据/知识/mixed 查询；同时覆盖 unavailable、失败、401、403、429、长任务、
空结果、无引用和后端断开。
...
建议 commit message 和 PR 摘要。不要自行 commit、push 或合并。
```
...
本批目标：在不提交权重/语料/索引/秘密的前提下，以只读外部挂载接收 A+B 与 D 的验收包，
在目标服务器完成无降级分析+RAG E2E、重启、备份恢复和回滚证据。
...
```text
你正在协助 NanoLoop-Agent。基线为 origin/yukun@16456a3。
...
3. 维护固定 demo 包和可重复的验收步骤；实际执行时记录命令、退出码和结果。
...
- 控制 PR 顺序，只把达到门禁的批次合入 `yukun`；阶段发布后再合入 `main`。
- 组织真实 E2E 和科学签字；任何一项没有证据都不以进度压力改写为“完成”。

## 10.2 每周集成清单

1. 更新团队看板：责任人、分支/PR、依赖、证据、风险和下一步。
2. 检查 `yukun` 是否全绿，并给开发者发送准确基线 commit。
...
```text
你正在协助我管理 Yukun-Zheng/NanoLoop-Agent，基线为 origin/yukun@16456a3。
...
| 里程碑 | 定义 | 必须具备的退出证据 |
| --- | --- | --- |
| G0 / v4 基线 | 所有人从同一全绿 `yukun` 开始 | commit 固定；CI、OpenAPI、迁移和文档一致 |
...
1. `feat/a-real-unet-large-v1`：模型资产身份和真实接入的最小阻塞修复。
2. `feat/b-scientific-tolerance-v1`：固定 GT、容差、统计和失败案例。
3. `feat/d-rag-real-assets-v1`：真实语料/embedding/index 及检索评测。
4. `ops/c-target-server-v1`：私有资产只读挂载、镜像和恢复。
5. `feat/e-real-demo-workflow-v1`：只修真实流程阻断与状态展示。
6. `test/f-nondegraded-e2e-v1`：固定演示包、真实 smoke 和证据归档。
...
| 门禁 | 主责 | 必须证据 | 阻断条件 |
| --- | --- | --- | --- |
| 基线 | C/郑煜坤 | 干净基线 commit、当前 CI、变更摘要 | 工作区来源不明或必需 CI 失败 |
...
11. 未经我明确授权，不 commit、push、建 PR、合并、删除分支或操作远端。
