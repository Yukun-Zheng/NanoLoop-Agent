...
- API 已拒绝不受信任/歧义 Host，并对浏览器写请求校验 Origin/fetch metadata；共享 Key 与可撤销 principal credential authentication 均已接通，tenant/principal/credential 状态由数据库和运维 CLI 管理。当前 principal 仅提供调用者上下文，尚未把模型、语料、任务或文件绑定 owner，也没有路由角色策略、租户隔离、分布式限流、调用 quota 或磁盘 quota/retention。不要将 `NANOLOOP_BIND_HOST` 改为公网地址并宣称 production-ready；远程访问至少需要受信任反向代理上的 TLS、所需的用户登录/联邦身份、完整业务授权、边缘限速和访问审计，多实例还需替换单机状态协调。
