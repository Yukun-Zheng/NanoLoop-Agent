...
| 基线日期 | `2026-07-18` |
| 应用版本 | `0.1.0` |
| 应用源码摘要 | `2b46e1238fc0e4b51d270a880d54d08b835a31d24b4cf1efeae7ccd7f5764493` |
| v2 参考文档 SHA-256 | `13ed57e1f1c82b0e6b7503f6aead8af061ef854e6d5fa03e1107e420d0c618da` |
| 验证快照 | Ruff、严格 Mypy、OpenAPI、435 项 Pytest、6 页 Streamlit 启动、Alembic 往返与漂移检查均通过 |
...
| 开发者 C | API、数据库、存储、调度与安全 | `app/main.py` | 用户/租户授权、配额、备份与多实例演进 |
...
并增加 Host/Origin、可选共享 `X-API-Key`、单进程固定三桶限流、请求体、multipart、
图像/PDF/RAG 规模和确定性证据上限。OpenAPI、前端 client、smoke、Compose 和 CI 已同步这一部署
合同；共享 Key 启用时，Streamlit 还会把 API 目标锁定到运维配置的 `NANOLOOP_API_BASE_URL`。GitHub CI
构建 API 与前端容器，再置于同一隔离网络验证 DNS/Host/Key 跨容器链路；官方 Actions 引用已固定到 commit SHA。
开发者不得把这些已经落地的保护降回 v2 的简化示例。

## 0.4 当前能力状态

| 范畴 | 状态 | 可依赖结论 | 不得误报的边界 |
| --- | --- | --- | --- |
| API / DB / 存储 | 已实现 | FastAPI、SQLite/Alembic、审计事件、内容寻址导出、受控下载、可选共享 Key 和单进程限流均有测试 | 尚无用户/租户登录与资源授权、分布式限流、用户配额和共享数据库 |
...
| 部署 / QA | 已实现单机基线 | GitHub CI 构建 API/前端 CPU 镜像，以非 root、只读根启动，并在隔离网络验证前端容器到 API 的 DNS/Host/Key 链路 | 本地 Docker image build 曾因外部镜像拉取超时；仍无生产备份/恢复演练与多实例证据 |
...
六页工作台由 `frontend/app.py` 组织：任务创建、ROI、模型选择、运行监控、结果与质量、知识与问答。`frontend/api_client.py` 是唯一后端入口；它从进程环境接收可选部署 Key，并统一附加到 JSON、multipart 与同源签名下载请求。配置共享 Key 时，连接地址由 `NANOLOOP_API_BASE_URL` 锁定，`_get_client()` 对会话值与运维值分别规范化后要求完全相等；不匹配则不创建 client，避免把进程级 Key 发往会话用户可控的地址。未配置 Key 的开发模式仍允许编辑 API 地址。ROI canvas 运行时不依赖 CDN/npm；结果页优先展示质量结论，再展示数值和五层图像。
...
- 配置 `NANOLOOP_API_KEY` 时，除精确公开路径外的请求必须只携带一个有效 `X-API-Key`；缺失、错误或重复头部统一返回 `401 AUTHENTICATION_REQUIRED`，不暴露匹配细节。合法 CORS 预检由外层 CORS middleware 直接响应；没有 `Origin`/`Access-Control-Request-Method` 的普通 `OPTIONS` 不获得认证或限流豁免。
- 精确公开路径是根 `GET /health`、OpenAPI/Swagger/ReDoc 文档路径；`GET /api/v1/health` 和 `/api/v1/files/{token}` 与其他版本化 API 一样受保护，相似前缀不会获得豁免。
- `API_RATE_LIMIT_REQUESTS > 0` 时启用单进程 token bucket，桶命名空间固定为 `service`、`authenticated`、`anonymous`，最多持有三个桶。成功响应带 `X-RateLimit-Limit/Remaining`，超限返回可重试的 `429 RATE_LIMITED` 与 `Retry-After`。
- 有效请求的中间件顺序由外到内为：Request Context → Trusted Host → Browser Mutation Guard → CORS → 限流（启用时）→ API Key → 请求体上限 → route。Host/Origin 拒绝先于 Key，Key 拒绝先于 body 解析。
- 共享 Key 同时配置到 Streamlit 时，前端必须把规范化后的请求地址锁定到 `NANOLOOP_API_BASE_URL`，不允许会话状态把 Key 改投到其他 origin/path。没有共享 Key 时才保留可编辑地址的本地开发行为。
- 共享 Key 只是单一部署密钥，不是用户/租户身份、角色或资源授权；三桶限流也不跨进程共享。开放公网前仍必须由受信任反向代理提供 TLS、真实身份/授权、分布式限速和访问日志。
...
| REST 路径 / 状态码 / multipart | routes、API client、OpenAPI（含 security/responses）、HTTP fixtures、集成测试、部署限制说明 |
...
- 统一响应/错误/request ID、可信 Host、浏览器写请求保护、CORS、可选共享 `X-API-Key`、请求体与 route-specific multipart 限额已接通。
- `ApiKeyVerifier` 仅保留 Key 的 SHA-256 摘要并做恒定时间比较；缺失、错误或重复头部有不可区分的 401 响应，所有版本化操作在 OpenAPI 声明 `ApiKeyAuth`。
- 可选 `TokenBucketLimiter` 按 `service/authenticated/anonymous` 三个固定调用类别限流，内存随请求来源数量保持常数；它只对当前进程生效。
- 安全中间件保持 Host/Origin 先于限流和 Key，Key 先于 body/multipart 解析；CORS 还对浏览器暴露 401/429 所需响应头。
- Compose 的显式 `TRUSTED_HOSTS` 默认项包含内部服务名 `api`，使前端可经 Compose DNS 访问后端；这是精确 Host 允许项，不是 wildcard。
...
| 文件/目录 | 关键对象 | 修改场景 |
| --- | --- | --- |
| `app/main.py` | `create_app`、服务装配、lifespan、中间件顺序 | 新基础设施、启动/关闭与依赖注入 |
| `app/api/routes/` | 14 类 REST 操作 | HTTP 解析、状态码与响应映射 |
| `app/api/deps.py` | `ApiKeyAuth`、`require_api_key_contract` | OpenAPI 安全声明与路由 fail-closed 复核 |
| `app/api/routing.py` | `BoundedMultipartRoute` | 文件/字段数、part size 与类型政策 |
| `app/api/middleware.py` | Host/Origin/API Key/限流/body/context middleware | 请求安全、限额、拒绝顺序和日志上下文 |
...
| `app/core/config.py`、`security.py`、`rate_limit.py`、`logging.py` | Settings、Key 验证、固定三桶限流与结构化日志 | 环境合同与生产边界 |
| `docs/adr/0004-shared-api-key-and-single-process-rate-limit.md` | 当前安全决策、顺序与局限 | 认证/限流更换前先写新 ADR |
...
7. 共享 Key 启用时，版本化 API、签名文件下载和版本化 health 不得绕过验证；公开豁免必须是精确路径，只有外层 CORS middleware 识别的合法预检可直接响应。
8. 不得记录、回显或把原始 Key 放入错误 details；后端 verifier 只保留摘要，重复 Key 头部必须 fail closed。
9. 中间件顺序、CORS 响应头和 OpenAPI `ApiKeyAuth`/401/403/429 是公开合同；修改时同步 HTTP 合同测试。
10. 进程内限流不得按 IP/Key 动态无界建桶，也不得声称为多 worker/多副本全局配额。
11. Compose/容器 DNS 名必须显式进入 `TRUSTED_HOSTS`；不得用 `*` 为了便利跨容器访问而关闭 Host 边界。
...
| 优先级 | 任务 | 完成证据 |
| --- | --- | --- |
| C-P0 | 用用户/租户身份与资源授权取代共享 Key 边界 | API/文件下载按主体和所有权授权；登录、匿名、撤销、错误与审计有测试 |
| C-P0 | 增加租户/用户配额与保留策略 | 数据所有权、容量上限、可恢复删除和审计契约明确 |
| C-P0 | 增加备份/恢复与灾难演练 | DB、制品、模型 snapshot、知识索引版本可一致恢复；定期演练 RPO/RTO |
| C-P1 | 设计 PostgreSQL + 共享对象存储 + 外部任务队列 | ADR、迁移工具、幂等 worker ownership 和回滚演练 |
| C-P1 | 将单进程三桶升级为分布式主体限流 | 多副本共享计数、按主体/操作分级、时钟/故障策略和容量测试有证据 |
| C-P2 | 增加 OpenTelemetry/指标 | request/run/job/queue 相关性完整，不记录秘密或二进制 |

## C.7 最低测试与交接

```bash
.venv/bin/pytest -q tests/unit/api tests/unit/core/test_security.py tests/unit/core/test_rate_limit.py
.venv/bin/pytest -q tests/unit/db tests/unit/storage tests/unit/orchestration
...
- `frontend/api_client.py` 统一处理 envelope、错误、multipart、下载和查询；`X-API-Key` 只由 client 内部注入，调用方不能通过通用 headers 伪造或替换它。
- `frontend/app.py` 只从 `NANOLOOP_API_KEY` 进程环境取 Key；client cache key 只保存 SHA-256 指纹，环境 Key 轮换会重建 client。
- Key 启用时，侧边栏的后端地址被 `NANOLOOP_API_BASE_URL` 锁定且不可编辑；`_get_client()` 再次规范化并完全比对会话/运维地址，因而被篡改的 session 无法获得带 Key 的 client。
- Key 未启用时保留原有开发体验：用户可编辑、规范化并保存 API 地址。
...
| 文件 | 关键职责 | 修改场景 |
| --- | --- | --- |
| `frontend/app.py` | 页面路由、业务流程、API 地址锁定与 client cache | 新页面、工作流、Key 轮换/目标绑定和可见状态 |
| `frontend/api_client.py` | REST 唯一入口与 `X-API-Key` 注入 | API 合同变化、认证错误、上传、轮询与下载 |
...
7. 部署 Key 不得作为明文字段进入 `st.session_state`、URL、页面、错误文本或日志；client cache 身份只使用指纹，原值封装在 API client 内。
8. 前端必须保留 `AUTHENTICATION_REQUIRED` 和 `RATE_LIMITED` 的服务端语义；对可变更请求不因 `retryable=true` 而无条件自动重放。
9. 只要进程配置共享 Key，任何 session 控制的 API 地址都不得越过规范化后的 `NANOLOOP_API_BASE_URL` 完全匹配；失配时先丢弃旧 client，再可见拒绝创建。
...
| E-P1 | 完善 401/429 与 Key 轮换体验 | 401 引导运维更新环境；429 显示等待信息，只对可安全重试的读请求按策略重试 |
| E-P1 | 扩展共享 Key 目标绑定回归 | 覆盖等价 URL 规范化、带 path 部署、非法运维值、session 篡改与 Key 轮换；错误中不泄密 |
...
- `.github/workflows/ci.yml` 在 Python 3.11/3.12 测试，并构建 API/前端 CPU 镜像；GitHub `main` 基线已通过两个镜像的实际启动验活。
- 容器 job 用非 root、只读根、`cap-drop ALL` 和 `no-new-privileges` 启动 API/前端；API 先在容器内用 CI 专用 Key 验证受保护的 `/api/v1/health`，前端也验证自身 `/_stcore/health`。
- API 与前端运行在同一个临时隔离 Docker 网络，API 使用 `api` 网络别名且 `TRUSTED_HOSTS` 显式允许该名。前端容器携带同一合成 Key 实际请求 `http://api:8000/api/v1/health`，一次覆盖 Docker DNS、Host allowlist 和 Key 跨容器传递。
- 当前 workflow 中 `actions/checkout` 和 `actions/setup-python` 均固定为官方 release 的完整 commit SHA；更新时必须记录对应 tag 与审核依据。
- `docker-compose.yml` 明确只读模型 bundle、可写 snapshot/output/knowledge volumes、cap drop、no-new-privileges 和健康检查。
- `scripts/smoke_test.py` 定义黑盒闭环，通过 `--api-key` 或 `NANOLOOP_API_KEY` 调用受保护 API；无外部资产时只能用 `--allow-degraded` 验证诚实降级。
...
1. 普通测试不得依赖外网、私有权重、生产语料、外部注入/生产 API Key 或特定 GPU；安全合同测试使用固定合成 Key。
...
7. 第三方 Actions 只能引用审核过的完整 commit SHA；版本 tag 仅作注释，不得退回浮动 `@vN`。
8. CI 中的 API Key 必须是专用合成值，不复用部署密钥；日志、失败输出和测试 snapshot 不得包含原始秘密。
9. OpenAPI 快照必须保留 `ApiKeyAuth` header scheme、全部版本化 operation 的 security requirement 以及 401/403/429 错误响应。
10. 容器 CI 不得只用 localhost 分别验进程；至少一个请求必须从前端容器通过服务 DNS 携 Key 到达受保护 API，并保持精确 `TRUSTED_HOSTS` 而非 wildcard。

## F.5 下一步任务优先级

| 优先级 | 任务 | 完成证据 |
| --- | --- | --- |
| F-P0 | 持续维护 GitHub 发布门禁与可追溯批次 | 短分支、提交、Draft PR、必需 CI、回滚信息和变更摘要完整 |
| F-P0 | 接入 A/D 的真实资产验收作业 | 受控 runner、外部资产注入、哈希/许可与不降级 smoke |
| F-P0 | 解决公开分发许可证边界 | 项目许可证与 Ultralytics、PyMuPDF 等可选依赖政策明确 |
| F-P1 | 把已有跨容器 health 扩展为业务请求/安全头 E2E | 在已验证 DNS/Host/Key 基础上，覆盖 Streamlit 实际 client、上传、读状态、签名下载和 CORS/限流响应头；可使用可授权固定 fixture |
| F-P1 | 增加依赖/镜像/SBOM/秘密扫描 | 失败政策、例外流程和发布制品签名明确 |
| F-P1 | 固定可复现依赖与基础镜像 | constraints/lock、基础镜像 digest、已固定 Actions 的更新流程明确 |
...
```bash
NANOLOOP_API_KEY='<deployment-key>' python scripts/smoke_test.py \
  --base-url http://127.0.0.1:8000 \
  --fixture demo_data/smoke_fixture.example.json
```

`--api-key` 可显式覆盖环境值；CI 和文档不得使用真实部署 Key 作示例。
...
所有版本化路径均位于 `/api/v1`，并在 OpenAPI 声明 header 型 `ApiKeyAuth`。只有配置
`NANOLOOP_API_KEY` 时才执行共享 Key；没配置时保持本地开发兼容。`GET /health` 是精确公开的无前缀
容器健康入口，而表中 `/api/v1/health` 在 Key 启用时需要 `X-API-Key`。OpenAPI/Swagger/ReDoc 路径同样
精确公开；签名文件 URL 仍需 Key，token 不替代部署认证。
...
除 v2 已列错误外，当前消费者还必须处理：`AUTHENTICATION_REQUIRED`、`RATE_LIMITED`、
...
`CROSS_SITE_MUTATION_FORBIDDEN`。`AUTHENTICATION_REQUIRED` 使用 HTTP 401 与 `WWW-Authenticate`；
`RATE_LIMITED` 使用 HTTP 429、`retryable=true`、`Retry-After` 与 `X-RateLimit-*` 头。证据不足
通常是 HTTP 200 的业务结果，不应误当传输异常。
...
| `TRUSTED_HOSTS` | local/test hosts；Compose 追加 `api` | C | 显式 Host allowlist；内部服务 DNS 名也必须精确列出 |
| `CORS_ALLOW_ORIGINS` | 空 | C/E | 浏览器写请求允许 origin |
| `NANOLOOP_API_KEY` | 空（关闭） | C/E/F | 可选共享部署 Key；32～128 位 URL-safe 字符，只在 TLS 后使用 |
| `NANOLOOP_API_BASE_URL` | `http://127.0.0.1:8000`；Compose `http://api:8000` | E/F | Streamlit 后端目标；配置共享 Key 时是不可被 session 改写的运维锁定值 |
| `API_RATE_LIMIT_REQUESTS` | Settings `0`；Compose `120` | C/F | 每个固定类别桶的容量；与下一项共同决定连续补充速率，`0` 关闭 |
| `API_RATE_LIMIT_WINDOW_SECONDS` | `60` | C/F | token bucket 从空到满的补充时长；进程内生效，不在副本间共享 |
...
| API 安全 | `.venv/bin/pytest -q tests/unit/core/test_security.py tests/unit/core/test_rate_limit.py tests/unit/api/test_middleware.py tests/unit/frontend/test_api_client.py tests/integration/api_contract` | Key 成败/目标绑定、CORS 预检、三桶隔离、429 头和 OpenAPI security 均符合合同 |
...
| 容器 CI | GitHub `CPU container smoke test` | API/前端以非 root、只读根启动；前端容器在隔离网络经 `api` DNS/Host 携测试 Key 访问版本化 health |

`make check` 已聚合代码质量、类型、OpenAPI、全量 Pytest、前端与迁移门禁；API 安全用例包含在全量 Pytest 中。Compose 和容器运行证据需另行执行。
