...
v4.0 的代码事实基线是 `yukun@16456a3`；最新协作状态以 `yukun` 分支 HEAD 和本索引所列开发日志为准。当前发布等级是 **M1 工程 MVP / 内部 Alpha**。仓库已具备完整工程骨架、六页前端和自动化门禁，但五个登记模型仍为 `unavailable`，正式模型/数据资产、许可语料、固定 embedding、真实向量索引和无降级 E2E 尚未完成。
