...
| 场景 | 当前结论 | 主要依据 / 阻塞 |
| --- | --- | --- |
| 本机或受信任内网、单 API 实例 | 支持 | [Compose](../docker-compose.yml) 默认回环绑定；SQLite WAL、持久 `QUEUED` 行、原子领取和有界 worker pool 已实现。 |
...
| 公网或多租户服务 | 不支持 | 应用没有账号/租户、认证授权、rate limit、调用/磁盘 quota、retention 和租户隔离。 |
| 多 Uvicorn worker / 多 API replica | 不支持 | SQLite 写协调、进程内 dispatcher、Adapter 缓存和导出协调按单进程/单 API 实例设计。 |

## 数据权威与可恢复投影

SQLite 是任务、图像、运行、颗粒、汇总、查询审计、活动 ROI 和 ROI revision 历史的权威
...
- `/health` 的 database component 除连通性外还比较 `alembic_version` 与打包迁移 head；缺表、
  缺 revision 或 stale revision 会报告 `unavailable`。

仍需在公网或长期运行前完成：

- 受信任反向代理上的 TLS、用户认证、授权、rate limit 和访问审计；
- 用户/任务/磁盘 quota、制品与语料 retention；
...
当前本地 ROI browser smoke 已在 headless Chrome 完成。`docker compose config --quiet` 只证明
Compose 配置可解析；本轮 Docker image build 在拉取 Docker Hub 基础镜像时超时，未取得成功
构建证据，不能将其列为已通过门禁。
