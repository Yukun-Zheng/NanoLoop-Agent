...
| 公网或多租户服务 | 不支持 | 可选共享 API Key 和单进程固定桶限流已接通，但没有账号/租户、角色授权、分布式限流、调用/磁盘 quota、retention 和租户隔离。 |
...
- 可选 32～128 位 URL-safe `NANOLOOP_API_KEY` 使版本化 API、multipart 与签名文件下载统一校验 `X-API-Key`；根健康探针和 API 文档使用精确豁免；
- 可选单进程 token bucket 将合法 Key、匿名/错误 Key、未启用认证的服务请求分成三个固定桶，返回有界 `429`/`Retry-After` 合同；
...
- 受信任反向代理上的 TLS、用户认证/授权、边缘/分布式 rate limit 和访问审计；
