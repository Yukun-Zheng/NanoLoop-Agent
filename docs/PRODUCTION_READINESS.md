...
| 公网或多租户服务 | 不支持 | 可撤销 principal credential、AnalysisJob tenant/owner 事实与 Analysis 聚合的 tenant/role/owner 授权已接通；但 query actor/深层数据工具隔离、tenant/job-bound 文件 token、知识文档租户化、分布式限流、调用/磁盘 quota 和 retention 尚未完成。 |
...
- 受信任反向代理上的 TLS；若场景需要交互式登录或联邦身份，还需独立的用户认证；业务层仍须完成
  query actor/数据工具深层 tenant scope、file-token v2 下载授权、知识资源租户化、边缘/分布式 rate
  limit 和访问审计；
