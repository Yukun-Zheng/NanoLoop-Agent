...
先在左侧勾选要纳入作用域的终态运行并进入“结果”，再在右侧“问答”标签中提问。面板会明确显示
当前图像、运行数量、材料和本地模型状态；空白页会直接提示下一步，不再要求第一次使用者猜测功能。
默认使用“自动判断（推荐）”，数据/知识/混合模式和可选材料纠正只在“高级选项”中：
...
最近一次对话和消息历史仍可重载，右上角“新对话”可开始新的上下文。平均等效粒径可另问 `平均等效粒径是多少？`，周长密度可问
...
回到材料名称为 `LaNi` 的任务，在结果页右侧打开“问答”，保持同一对话并使用默认自动模式。
...
回到“结果”，选中 1～3 个终态运行并点“生成分析报告”。预期网页先显示技术摘要、关键指标、
运行对比、证据化发现、带理由和验证方式的行动建议、方法、限制、待回答问题和可复现来源；同一区域
同时内嵌 PDF 预览，并提供 DOCX/PDF 下载。若图像缺少比例尺，报告应明确显示“仅像素尺度”，不出现
推断得到的物理单位。浏览器在下载 DOCX/PDF 前会核对服务端声明的 SHA-256。

需要完整证据包时，再点“导出当前及所选运行”。浏览器完成下载前同样核对 SHA-256，成功时显示
“SHA-256 已验证，可信报告已下载”。
