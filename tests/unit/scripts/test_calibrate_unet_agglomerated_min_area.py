...
def _write_fixture(tmp_path: Path) -> tuple[CalibrationPaths, dict[str, str]]:
    ...
    for filename in VALIDATION_FILENAMES:
        ...
        Image.fromarray(targets[filename].astype(np.uint8) * 127).save(image_path)
        artifacts[filename] = {
    ...
    evidence = {
        ...
        "inference_config": {"path": "config.yaml", "sha256": "a" * 64},
        "execution_evidence": {
            filename: {
                "actual_device": "cpu",
    ...
    evidence_path = threshold_root / "threshold-calibration.json"
...
def test_fixed_read_only_contract_and_cli() -> None:
    ...
    assert DEVELOPER_REPORTED_THRESHOLD_EVIDENCE_SHA256 == (
...
def test_rejects_unfrozen_or_wrong_validation_evidence(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="frozen agglomerated contract"):
        _load_threshold_evidence(
            ...
            expected_sha256=_sha256(evidence_path),
        )


def test_requires_explicit_matching_evidence_digest_and_execution_identity(
    tmp_path: Path,
) -> None:
    paths, _ = _write_fixture(tmp_path)
    ...
    payload["execution_evidence"].pop(VALIDATION_FILENAMES[0])
    evidence_path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises(ValueError, match="missing execution identity"):
        _load_threshold_evidence(
            paths.threshold_calibration_root,
            expected_sha256=_sha256(evidence_path),
        )
...
def test_four_field_run_reuses_and_does_not_modify_probability_cache(tmp_path: Path) -> None:
    ...
    payload = run(paths, expected_size=(80, 160))
...
def test_mask_or_probability_hash_mismatch_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="mask identity mismatch"):
        run(paths, expected_size=(80, 160))
...
def test_parser_requires_only_read_only_sources() -> None:
    namespace = build_parser().parse_args(
        [
            ...
            "--threshold-evidence-sha256", "a" * 64,
            "--image-dir", "images",
            "--mask-dir", "masks",
            "--output-root", "output",
        ]
