...
默认只绑定 `127.0.0.1`。API 会拒绝不受信任/歧义的 Host，并对浏览器写请求校验 Origin 与 `Sec-Fetch-Site`；这只能降低 DNS rebinding 和跨站写入风险，不等于身份认证。若要开放到其他机器，仍必须先在受信任反向代理上增加认证、TLS、请求限速和访问日志，再显式设置 `NANOLOOP_BIND_HOST`；当前应用自身不提供用户登录体系。
...
本地 ROI browser smoke 已使用 headless Chrome 完成真实拖拽 → CAS 保存 → 页面重载 → REST 数据核对。`docker compose config --quiet` 可校验 Compose 配置，但本环境的镜像构建在拉取 Docker Hub 基础镜像时超时，尚不能记为 Docker image build 通过。
