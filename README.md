...
- 科研助手支持任务内多轮对话、历史重载和通用对话优先路由。本地 Qwen3 可回答普通交流、
  ...
  工具。材料与文献问题会同时检索本地知识库和 Crossref 学术题录；配置 `TAVILY_API_KEY`
  后还会补充通用网页结果，回答中保留可点击来源、工具调用记录和证据限制。外部服务只接收
  本轮问题文本，不接收图像、运行记录或模型制品；Crossref 题录/摘要和网页摘要用于发现与
  有限证据，不等同于取得论文全文或完成系统综述。每个当前实验数值句必须引用 `[D#]`，
  证据模式中的材料事实句必须引用 `[C#]`。模型不可用、JSON/引用/数值/单位校验失败时自动
  回退可信模板或摘录，并保存
  `fallback_used`、模型身份、耗时和版本化 prompt 摘要，不保存思维链。
- 独立的科研 Agent 控制平面把本地模型用于持续规划，而不再只用于润色回答。模型每轮只能选择
  一个结构化动作；运行时独立执行参数校验、权限、预算、人工审批和证据校验。任务、审批和公开
  事件轨迹持久化，后台分析使用不消耗规划步数的只读续查。决策模型可通过 `AGENT_MODEL_*`
  单独替换，当前工具覆盖任务检查、模型推荐、分析/复核、数据查询、科研报告和复现包。架构和
  扩展合同见[科研 Agent 控制平面](docs/AGENT_CONTROL_PLANE.md)。
  续查由 API 进程内调度器按持久唤醒时间执行，关闭浏览器不会中断；续跑前会重新检查创建凭据
  是否仍有效。进程在非幂等写动作开始后崩溃时不会静默重放该动作。固定的无隐私控制器评测集
  同时检查动作、生产工具参数和意外写入；本机 Qwen3 4B 在 v2 控制提示下通过 10/10 个核心
  决策场景，复现命令和适用边界见控制平面文档。
- 科研助手位于结果页右侧审查器，并自动绑定当前图像和所选运行；结果区可基于最多 20 个终态运行
  生成自适应报告。单图报告保留当前形貌与质量解释；跨图批量报告额外给出均值、标准差、四分位数、
  极值、CV、质量分层和 1.5×IQR 异常视野。颗粒统计和质量判断由确定性数据工具提供，本地 Qwen3
  只综合已验证证据；报告同时提供网页预览、DOCX 和内嵌 PDF，下载前校验 SHA-256。缺少比例尺时
  会明确标记为像素级限制，不把 `px` 推断成 `nm` 或 `µm`。
...
- 结果页先展示质量结论、原因和建议，再展示数值；单运行可切换原图、mask、overlay、实例标注和概率制品，不可直接预览的 TIFF/数组只提供下载审查。工作区在单图任务中沿用当前图，在多图任务中默认一次提交全部图像，并可随时切回仅当前图；每批最多 20 张、最多 3 个模型，后端创建完整 image × model 运行集合并由有界 worker pool 调度。终态运行可跨图加入批量汇总。Large 与 Small-A 已在同一公开合成图完成 live 编排和浏览器并排比较，但仍缺共同授权 SEM/GT、固定科学容差和当前 bundle 的科学复验，不能据此选择“最佳模型”。
...
数据问答 → 网页/DOCX/PDF 科学报告 → 确定性 ZIP` 链路：
...
覆盖创建任务、ROI revision/CAS 恢复、ready/unavailable 模型、运行/结果/复核/右侧作用域问答、
科学报告、签名导出、响应式审查器和知识库生命周期；此外 2026-07-23/24 已在 macOS 本机用当前 Next.js、
