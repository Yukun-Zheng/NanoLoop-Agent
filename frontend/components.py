...
def render_query_response(streamlit: Any, response: Mapping[str, Any]) -> None:
    ...
    # --- 材料知识引用 区块 ---
    # citations 位于 UnifiedQueryResponse 顶层（徐皓彬 D 确认的真实结构），
    # 每个 citation 的 page 字段可能为 None（全文引用），需显式标注而非留空。
    citations = response.get("citations")
    has_citations = isinstance(citations, list) and citations
    ...
    if has_citations:
        ...
        streamlit.info(
            "未返回材料知识引用；当前回答中涉及材料背景的陈述不应视为已有文献来源支持。"
        )

    # --- 实验数据结论 区块 ---
    evidence = response.get("data_evidence")
    has_evidence = isinstance(evidence, list) and evidence
    ...
    if has_evidence:
        ...
        streamlit.info(
            "未返回实验数据结论；当前回答不包含来自确定性分析工具的数值证据。"
        )
...
def _error_guidance(code: str, status_code: Any) -> str | None:
    ...
    if normalized_code in {"SERVICE_UNAVAILABLE", "MODEL_NOT_READY", "RAG_INDEX_NOT_READY"} or sc == 503:
    ...
    if normalized_code == "REVIEW_REQUIRED":
        return "质量门禁判定该结果需要人工复核。请在运行结果页面查看质量原因，并在复核子运行中提交修正。"
