...
def build_actionable_error_html(
    *,
    ...
    request_html = (
        '<span class="nl-error-request-id">request_id '
        f"<code>{html.escape(request_id)}</code></span>"
        if options.show_request_id and request_id
        else ""
    )
    hint_html = f'<div class="nl-error-hint">{html.escape(hint)}</div>' if hint else ""
    recovery_html = ""
...
def build_service_unavailable_html(
    *,
    ...
    meta_html = f'<div class="nl-error-meta">{code_html}</div>' if code_html else ""
    return (
        '<div role="alert" class="nl-error-panel nl-error-panel-bad">'
